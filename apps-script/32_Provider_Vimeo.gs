/** ARCHIPIÉLAGO VIVO TV · PROVEEDOR VIMEO */
...
function refreshTvMediaVimeoMetadataInternal_() {
  ...
  const records = tvReadObjects_(sheet).filter(function(record) {
    return tvNormalizeProvider_(record.provider) === 'vimeo';
  });
  ...
  records.forEach(function(record) {
    ...
    try {
      ...
      tvPatchMediaTechnical_(record._rowNumber, {
        provider: 'vimeo', provider_id: identity.provider_id, provider_url: identity.provider_url,
        embed_url: embedFromHtml, title: data.title || record.title || '',
        ...
        embeddable: true, metadata_updated_at: new Date()
      });
      updated++;
    } catch (error) {
      console.warn('Vimeo ' + identity.provider_id + ': ' + error.message);
      unavailable++;
    }
  });
